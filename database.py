...
def view_accounts() -> None:
    ...
    for row in cursor.fetchall():
        print(f"Account id: {row[0]}, name: {row[1]}, amount: {row[2]}")
...
def add_account(account_name: str, account_amount: float) -> None:
    ...
    cursor.execute("INSERT INTO accounts (name, amount) VALUES (?, ?);", (account_name, account_amount))
...
def add_expense(expense_name: str, expense_amount: float, account_id: int) -> None:
    ...
    try:
        cursor.execute("INSERT INTO expenses (name, amount, account_id) VALUES (?, ?, ?);", (expense_name, expense_amount, account_id))
        cursor.execute("UPDATE accounts SET amount = amount - ? WHERE id = ?;", (expense_amount, account_id))
